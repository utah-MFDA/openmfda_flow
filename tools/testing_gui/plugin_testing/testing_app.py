...
class TestSubprocDialog(wx.Dialog):
    ...
    def __init__(self, cmd, *args, **kw):
        ...
        new_pnr_btn = wx.Button(panel, label='New PnR design')
        new_pnr_btn.Bind(wx.EVT_BUTTON, self.new_pnr_evt) 
        ...
        refresh_btn = wx.Button(panel, label='Refresh designs')
        refresh_btn.Bind(wx.EVT_BUTTON, self.refresh_designbox)
        ...
        sizer.Add(new_pnr_btn, 0, wx.ALL, 6)
        sizer.Add(pnr_btn, 0, wx.ALL, 6)
        sizer.Add(render_btn, 0, wx.ALL, 6)
        sizer.Add(slice_btn, 0, wx.ALL, 6)
        
        sizer.Add(refresh_btn, 0, wx.ALL, 6)

        panel.SetSizer(sizer)

        self.SetInitialSize(wx.Size(350, 410))

    def new_pnr_evt(self, event):
        sub = eeschema_2_mfda_dialog.ImportSchematicDialog(
            '',
    ...
    def refresh_designbox(self, event=None):
        pass
