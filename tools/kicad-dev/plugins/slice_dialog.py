...
class SliceSubprocDialog(wx.Dialog):
    """
    A dialog that logs subprocess call
    """

    def __init__(self,
                 cmd_ren, cmd_sl, *args, **kw):
        super(SliceSubprocDialog, self).__init__(
        ...
        slice_btn.Bind(wx.EVT_BUTTON, self.slice)
    ...
    def slice(self, event):
        self.log.write(' '.join(self.cmd_sl))
        self.proc = subprocess.Popen(
            self.cmd_sl,
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
...
class SlicePlugin_test():
    def defaults(self):
        self.name = "OpenMFDA Slicer"
    ...
    def Run(self):
        self.defaults()
        sub = SliceSubprocDialog('', None, title=self.name)
        sub.ShowModal()


def test_slice_dialog():
    SlicePlugin_test().Run()
