...
# do not change placement data to evaluate both
def get_placement_data(results_dir_root, design, design_varient="base", placement_data=None):
    ...
    # fmt: off
    try:
        if placement_data is None or \
                placement_data.lower() in GLOBAL_PL:
            gp_pos = def_parse.get_info(gp_file, design, ["COMPONENTS", "PINS"])
        if placement_data is None or \
                placement_data.lower() in DETAIL_PL:
    ...
    elif placement_data.lower() in GLOBAL_PL:
        return (gp_pos)
    elif placement_data.lower() in DETAIL_PL:
        return (dp_pos)
    else:
        raise ValueError("Invalid placement_data (arg 4)")


def get_placement_distances(results_root, design, verilog_file, lef_files,
                            conversion_factor, design_varient='base', placement_data=None):

    def_data = get_placement_data(
        results_root, design, design_varient, placement_data)
    lef_list = []
    if isinstance(lef_files, str):
        lef_list = component_parse.ComponentParser().get_comp_pins_from_lef(lef_files)
    if isinstance(lef_files, list):
        lef_list = []
        for lef in lef_files:
            lef_list += component_parse.ComponentParser().get_comp_pins_from_lef(lef)
    ver_G = read_netlist.import_from_file(verilog_file)

    if placement_data is None or placement_data.lower() in GLOBAL_PL:
        nd_gp = traverse_graph(ver_G.graph, lef_list,
                               def_data[0], cv=conversion_factor)
    if placement_data is None or placement_data.lower() in DETAIL_PL:
        nd_dp = traverse_graph(ver_G.graph, lef_list,
                               def_data[1], cv=conversion_factor)

    if placement_data is None:
        return {"global_place": nd_gp["distances"], "detail_place": nd_dp["distances"]}, \
            {"global_place": nd_gp["center"], "detail_place": nd_dp["center"]}
    elif placement_data.lower() in GLOBAL_PL:
        return {"global_place": nd_gp["distances"]}, \
            {"global_place": nd_gp["center"]}
    elif placement_data.lower() in DETAIL_PL:
        return {"detail_place": nd_dp["distances"]}, \
            {"detail_place": nd_dp["center"]}
    else:
        raise ValueError("Invalid placement_data (arg 4)")
