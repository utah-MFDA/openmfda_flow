if {![info exists standalone] || $standalone} {
  ...
  # for antenna checker read_def -order_wires $::env(RESULTS_DIR)/4_1_fill.def
  # -order_wires flag is REQUIRED to run RCX
  if {[info exist ::env(RCX_RULES)]} {
    read_def -order_wires $::env(RESULTS_DIR)/4_1_fill.def
  } else {
    read_def $::env(RESULTS_DIR)/4_1_fill.def
  }
  read_sdc $::env(RESULTS_DIR)/4_1_fill.sdc
...
if {[info exist ::env(RCX_RULES)]} {
  ...
  # Write Spef
  write_spef $::env(RESULTS_DIR)/4_final.spef
  file delete $::env(DESIGN_NAME).totCap

  # Read Spef for OpenSTA
  read_spef $::env(RESULTS_DIR)/4_final.spef
