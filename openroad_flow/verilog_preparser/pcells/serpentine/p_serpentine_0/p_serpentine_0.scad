    ...
    no_obj=false, floor_area=false, chan_layers=1, rot=0, clr="RosyBrown")
{
    ...
    if(!no_obj)
        translate([xpos*px, ypos*px, zpos*px])
            translate((rot?[y_off, x_off, chan_h/2*layer]:[x_off, y_off, chan_h/2*layer])-[(rot?0:1),(rot?1:0),0]*chan_w*px/2)
                obj(orientation);
...
p_serpentine_0(100 ,0,0,"N", 200, 50, 4, floor_area=true, rot=0);